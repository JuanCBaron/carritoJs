...
(function($){
	...
	app.masVendidos = function(){
		...
		for(var i = 0; i < productos.length; i++){
			...
			if(productos[i].stock > 0){
				...
				contenido+= '		<button class="large-12 columns fas fa-star ladda-button prod-'+productos[i].id+'" data-style="slide-right"></button>'
				contenido+= '		<button class="large-12 columns fas fa-star ladda-button prod-'+productos[i].id+'" data-style="slide-right"></button>'
				contenido+= '		<button class="large-12 columns fas fa-star ladda-button prod-'+productos[i].id+'" data-style="slide-right"></button>'
				contenido+= '</div>'
				...
				contenido+= '			<h3><span>Categoria: </span>'+productos[i].category+'</h3>'
				contenido+= '			<h3>Unidades disponibles: <span class="stock">'+productos[i].stock+'</span></h3>'
				contenido+= '		</span>'
				contenido+= '		<a class="large-12 columns btn submit ladda-button prod-'+productos[i].id+'" data-style="slide-right" onclick="app.addtoCart('+productos[i].id+');">Agregar al Carrito</a>'
	...
	app.searchProd = function(cart,id,cant,name,price,img,available){
		...
		if(undefined != curProd && curProd != null){
			//ya existe el producto, a̱adimos uno mas a su cantidad
